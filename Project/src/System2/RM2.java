...
public class RM2 {
	...
	public  static String sendMessage(String data) {
		...
		try {
			...
			System.out.println("Request message sent from the client to server1 with port number  + 1012 +  is: "
	...
	public static void main(String[] args) throws IOException {
		...
		file= new File("RM2Log.txt");
		file.createNewFile();
		
		System.out.println("RM2 is Ready & waiting for client...");
		Runnable task1 = () -> {
			try {
				receive();
			} catch (IOException e) {
	...
	public  static void receive() throws IOException {
		...
		while(true) {
			...
			long msgNum = Long.parseLong(requestArray[requestArray.length-1].trim());
				...
				FileWriter serverFile=new FileWriter("RM2Log.txt",true);
	...
	private static String processRequest(String request) {
		...
		if(requestArray[1].equals("checkRegistration")) {			
				...
				deptType=deptType.toLowerCase();
				if(deptType.equals("comp")){
					Server1 SerVer1= new Server1();
					responseInt=SerVer1.checkRegistration(requestArray[2].trim());
				}
				else if(deptType.equals("soen")){
					Server2 SerVer2= new Server2();
					responseInt=SerVer2.checkRegistration(requestArray[2].trim());
				}
				else {
					Server3 SerVer3= new Server3();
					responseInt=SerVer3.checkRegistration(requestArray[2].trim());
				}	
			...
			if(requestArray[1].equals("getClassSchedule")) {
				...
				if(deptType.equals("COMP")){
					Server1 SerVer1= new Server1();
					response=SerVer1.getClassSchedules1(id);
				}
				else if(deptType.equals("SOEN")){
					Server2 SerVer2= new Server2();
					response=SerVer2.getClassSchedules2(id);
				}
				else {
					Server3 SerVer3= new Server3();
					response=SerVer3.getClassSchedules3(id);
				}
				System.out.println();
			}			
